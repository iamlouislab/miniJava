...
public class BinaryExpression implements Expression {
	...
	public Fragment getCode(TAMFactory _factory) {
		Fragment ret = this.left.getCode(_factory);
		if (this.left instanceof AccessibleExpression) {
			ret.add(_factory.createLoadI(this.left.getType().length()));
		}
		ret.append(this.right.getCode(_factory));
		if (this.right instanceof AccessibleExpression) {
			ret.add(_factory.createLoadI(this.right.getType().length()));
		}
		ret.add(TAMFactory.createBinaryOperator(this.operator));
		return ret;
	}
